...
- `src/app/[locale]/recruit/` … 求人ページ（**意図的に日本語のみ**。内容は管理画面の「求人情報/求人ブログ」タブ→Firestore `sakuraspa/jobs`・`jobPoints`・`jobBlog` から取得）
- `src/lib/spa-data.ts` … Firestore取得＋表示ヘルパ（therapists/schedule/banners/jobs）
...
## LLMO（AI検索最適化）
- `public/llms.txt`（AI向けサイト案内・料金や営業時間を変えたらここも更新）、`public/robots.txt`（AIクローラー許可）、`src/app/sitemap.ts`
- 構造化データは `src/lib/jsonld.ts`（トップ=LocalBusiness、求人=JobPosting）。**内容は必ずページ表示と一致させる**こと
- **求人内容を更新したら jsonld.ts の datePosted を当日に書き換える**（古い日付は評価が落ちる）
- 残タスク: FAQページ（客向け/求人向け・条件確定後）、冒頭300字の即答文（5言語）、Bing Webmaster Tools・Google Search Console 登録（ユーザー作業）

## 未完了（次の作業候補）
- 固定電話番号への差し替え（constants.ts の PHONE_TEL/PHONE_DISPLAY）
- 求人ページのコンテンツ整備（管理画面「求人情報」タブの初期値はVELOUR流用のため、SAKURA向けに書き換えて保存する）
